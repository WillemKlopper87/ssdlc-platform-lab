...
| T10 | No two-party *exception* workflow exists — a genuinely new Critical/High finding has no accepted-risk path other than a reviewed change to `policy/severity.rego` itself. (Baseline/differential gating for *pre-existing* findings is now built, SADR-0024/0025 — this row covers only the exception half, which is a separate, still-open gap) | Depends on exceptions-repo infrastructure, Milestone 4 | [`EXCEPTIONS.md`](EXCEPTIONS.md) |
